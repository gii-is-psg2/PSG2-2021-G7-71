...
public interface AdoptionRepository extends Repository<Adoption, Integer> {
    ...
    @Query(value = "SELECT a FROM Adoption a WHERE a.owner.id = :ownerId AND a.pet.id = :petId")
    Adoption findByOwnerAndPet(@Param("ownerId") Integer ownerId,@Param("petId") Integer petId) throws DataAccessException;
