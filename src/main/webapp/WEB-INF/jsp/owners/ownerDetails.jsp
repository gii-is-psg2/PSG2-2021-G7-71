...
<petclinic:layout pageName="owners">
	...
	<h2>
		<fmt:message key="ownerInformation" />
	</h2>
	...
	<h2>
		<fmt:message key="petvisitbooking" />
	</h2>
			...
			<tr>
				<td valign="top">
					<dl class="dl-horizontal">
						...
						<dt>
							<fmt:message key="petInformation" />
						</dt>
						<dd>
						...
						<dt>
							<c:choose>
								<c:when test="${pet.adoption}">
									<spring:url value="/owners/adoptions/{petId}" var="adoptionURL">
										<spring:param name="petId" value="${pet.id}" />
									</spring:url>

									<a href="${fn:escapeXml(adoptionURL)}"> <c:out
											value=" Solicitudes adopcion" />&nbsp;
									</a>
								</c:when>
							</c:choose>
						</dt>
						<dd>
							<c:out value="${pet.adoption ? 'Sí' : 'No'}" />
						<dt>
							...
							<a href="${fn:escapeXml(petUrl)}"><fmt:message
									key="updatePet" /></a>
						</dt>
						<dt>
							<spring:url value="/owners/{ownerId}/pets/{petId}/adopt"
								var="adoptUrl">
								<spring:param name="ownerId" value="${owner.id}" />
								<spring:param name="petId" value="${pet.id}" />
							</spring:url>
							<a href="${fn:escapeXml(adoptUrl)}"><fmt:message
									key="adoptPet" /></a>
						</dt>

						<dt>
							<spring:url value="/owners/{ownerId}/pets/{petId}/delete"
								var="deleteUrl">
								<spring:param name="ownerId" value="${owner.id}" />
								<spring:param name="petId" value="${pet.id}" />
							</spring:url>
							<a href="${fn:escapeXml(deleteUrl)}"><fmt:message
									key="deletePet" /></a>
						</dt>
						...
						<c:forEach var="visit" items="${pet.visits}">
							<tr>
								...
								<td><spring:url
									...
									</spring:url> <a href="${fn:escapeXml(deleteVisitUrl)}"><fmt:message
											key="deleteVisit" /></a></td>
							</tr>
						</c:forEach>
						<tr>
							<td><spring:url
					...
					<table class="table-condensed">
						...
						<c:forEach var="booking" items="${pet.bookings}">
							<tr>
								...
								<td><spring:url
										value="/owners/{ownerId}/pets/{petId}/bookings/{bookingId}/delete"
										var="deleteBookingUrl">
										<spring:param name="ownerId" value="${owner.id}" />
										<spring:param name="petId" value="${pet.id}" />
										<spring:param name="bookingId" value="${booking.id}" />
									</spring:url> <a href="${fn:escapeXml(deleteBookingUrl)}"><fmt:message
											key="deleteBooking" /></a></td>
							</tr>
						</c:forEach>
						<tr>
							<td><spring:url
									value="/owners/{ownerId}/pets/{petId}/bookings/new"
									var="bookingUrl">
									<spring:param name="ownerId" value="${owner.id}" />
									<spring:param name="petId" value="${pet.id}" />
								</spring:url> <a href="${fn:escapeXml(bookingUrl)}"><fmt:message
										key="addBooking" /></a></td>
						</tr>
					</table>
				</td>
